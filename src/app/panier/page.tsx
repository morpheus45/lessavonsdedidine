...
import { calculerPanier, type PanierCalcule } from '@/lib/panier-calcul';
import { SEUIL_LIVRAISON_OFFERTE_CENTIMES } from '@/donnees/catalogue';
...
export default function PagePanier() {
  ...
  // Le panier vit dans le navigateur : il n'est connu qu'après hydratation.
  // On part d'un état « pas encore chargé » explicite plutôt que d'afficher
  // « panier vide » puis de se corriger.
  useEffect(() => {
    const rafraichir = () => {
      const liste = lirePanier();
      setArticles(liste);
      setPanier(calculerPanier(liste));
      setCharge(true);
    };
    rafraichir();
    ...
    <>
      ...
      <main id="contenu" className="mx-auto max-w-[1000px] px-6 py-20">
        ...
        {panier && panier.obsoletes.length > 0 && (
          ...
          >
            Ces articles ne sont plus au catalogue et ont été retirés :{' '}
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr_360px]">
            <ul className="border-t border-brume">
              {panier.lignes.map((l, index) => {
                ...
                const id = article ? ligneId(article) : l.id;
                return (
                  <li
                    key={l.id}
            ...
            <aside className="h-fit rounded-l border border-brume bg-neige p-7">
              ...
              <p className="my-5 text-[13px] text-taupe">
                {panier.manquePourLivraisonOfferte > 0
                  ...
                  : `Seuil des ${formaterPrix(SEUIL_LIVRAISON_OFFERTE_CENTIMES)} franchi — livraison offerte.`}
              </p>
